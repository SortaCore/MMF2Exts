...
window['darkEdif'] = (window['darkEdif'] && window['darkEdif'].sdkVersion >= 16) ? window['darkEdif'] :
	...
	this.checkSupportsSDKVersion = function (sdkVer) {
		if (sdkVer < 16 || sdkVer > 16) {
			throw "UWP DarkEdif SDK does not support SDK version " + this.sdkVersion + " Bluewing Client does not support DarkEdif SDK version ";
		}
	};
...
function CRunBluewing_Client() {
	...
	this.Action_SaveReceivedBinaryToFile = function (position, size, fileName) {
		if (position < 0)
			...
			this.CreateError("File saving is not available in UWP. Ask for it");
	};
	this.Action_AppendReceivedBinaryToFile = function (position, size, fileName) {
		if (position < 0)
			...
			this.CreateError("File loading/saving by path is not available in UWP. Ask for it");
	};
...
function BluewingClient_EventToRun(ids, data) {
	...
	this.deniedClientName = data.deniedClientName || ""; // Note: no current way to read the denied name in ext version 101
