/*
	This Lacewing Blue Client Fusion extension HTML5 port is copyright © 2022 by Darkwire Software.
...
/// <reference path="https://cdn.jsdelivr.net/gh/inexorabletash/text-encoding@master/lib/encoding.js" />
...
// Main differences between Windows and HTML5:
// 1) UDP is not used, but is faked by part of the variant of the messages.
//    Browsers do not use UDP; the only possible other thing is WebRTC's timer-based "must send by x time
//    or message is discarded" system. WebRTC is not used by Blue.
// 2) No liblacewing event pump, checking the sockets and handling messages.
//    Sockets are maintained by the webpage anyway. However, there is some dummy code to more closely mirror Windows,
...
// 7) HTML5 and Windows do not share protocols. WebSocket and raw socket are too different.
...
function CRunBluewing_Client() {
	...
	// We need this[] and globalThis[] instead of direct because HTML5 Final Project minifies and breaks the names otherwise
	this['ExtensionVersion'] = 100; // To match C++ version
	this['SDKVersion'] = 16;
	...
	if (typeof globalThis['TextEncoder'] === 'undefined' || typeof globalThis['Zlib'] === 'undefined' || globalThis['Zlib']['RawDeflate'] === 'undefined') {
		...
		if (blueCliToScriptLoad.length == 1) {
			...
			if (typeof globalThis['TextEncoder'] === 'undefined') {
				...
				script2.src = 'https://cdn.jsdelivr.net/gh/inexorabletash/text-encoding@master/lib/encoding.js';
				script2.text = '/* Loaded TextEncoding library automatically from a secure ' +
					'cloud connection by request of Bluewing. */';
				document.head.appendChild(script2);
			}

			// Likewise, load ZLIB, which probably won't be in the browsers
			if (typeof globalThis['Zlib'] === 'undefined' || typeof globalThis['Zlib']['RawDeflate'] === 'undefined') {
				let script3 = document.createElement('script');
				script3.type = 'text/javascript';
				script3.src = 'https://cdn.jsdelivr.net/gh/imaya/zlib.js@master/bin/zlib.min.js';
	...
	// We need globalThis instead of Intl direct workarounds because HTML5 Final Project minifies and breaks the names otherwise
	...
	this.Expression_Lacewing_Version = function () {
		return "liblacewing 0.5.4 (HTML5 x86, 32-bit) / Bluewing reimpl b" + this['ExtensionVersion'];
	};
	...
	this.Expression_SendBinaryAddress = function () {
		this.CreateError("Send binary memory address is not relevant in a JS context");
		return "";
	};
	this.Expression_DumpMessage = function () {
		this.CreateError("Dumping binary messages not supported in HTML5 Bluewing, please " +
	...
	this.Expression_ConvToUTF8_TestAllowList = function (str) {
		this.CreateError("\"Convert to UTF-8 and test allowlist\" expression is not available in HTML5 port.");
		return "(not available in HTML5)";
	};
	// =============================
	// Macros
	// =============================
...
function BluewingClient_EventToRun(ids, data) {
	...
	this.deniedClientName = data.deniedClientName || ""; // Note: no current way to read the denied name in ext version 100
...
function Bluewing_Client(blue) {
	...
	const implStr = this.encoder.encode("Bluewing HTML5 b" + this.ext['ExtensionVersion']);
