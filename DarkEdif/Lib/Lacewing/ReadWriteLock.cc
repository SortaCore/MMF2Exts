...
#endif
{
	...
	if (writers)
		MsgBoxDeath(); // Shouldn't have a writer still locking while we're reading
	...
	for (auto& i : holders)
	{
		if (i.threadID == std::this_thread::get_id())
			MsgBoxDeath(); // multiple readers on same thread, first one should've been closed
	}
#endif
...
#ifdef _DEBUG
	if (readers == 0)
		MsgBoxDeath(); // don't own this lock!

	if (writers)
		MsgBoxDeath(); // writer shouldn't be accessing while a read lock is open...
...
#endif
{
	...
	if (readers)
		MsgBoxDeath(); // readers shouldn't be accessing while a write lock is open...
