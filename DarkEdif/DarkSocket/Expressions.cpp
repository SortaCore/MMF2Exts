...
{
	ThreadSafe_Start();
	std::tstring temp = CompleteStatus;
	if (clear)
...
{
	ThreadSafe_Start();
	std::tstring temp = (TCHAR *)Returns[0].Message;
	ThreadSafe_End();
...
char * Extension::LastMessage_GetString(size_t WhereFrom, size_t SizeOfString)
{
	if (SizeOfString > 0)
	{
		...
		if (Returns.size() > 0)
		{
			...
			{
				std::string s((char *)Returns[0].Message, WhereFrom, SizeOfString);
				ThreadSafe_End();
...
wchar_t * Extension::LastMessage_GetWString(size_t WhereFrom, size_t SizeOfString)
{
	if (SizeOfString > 0)
	{
		...
		if (Returns.size() > 0)
		{
			...
			{
				std::wstring s((wchar_t *)Returns[0].Message, WhereFrom, SizeOfString);
				ThreadSafe_End();
