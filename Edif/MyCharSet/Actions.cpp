...
void Extension::SetBOMMarkASC(const char * FileToAddTo, int TypeOfBOM, int IgnoreCurrentBOM)
{
	...
	BOMThreadData * Data = new BOMThreadData(FileToAddTo, TypeOfBOM, IgnoreCurrentBOM == 1);
...
DWORD WINAPI SetBOMMarkASCThread(void * Param)
{
	...
	if (Data->IgnoreCurrentBOM == false && FileSize >= 3)
	{
		...
		if (s != 3 && s != 2)
		{
			// Failed to read (BOM should be there)
			fclose(ReadFromFile);
			fclose(WriteToFile);
			return 4;
		}
	...
	if (Data->TypeOfBOM == 1)
	{
		unsigned char UTF8_BOM[2] = { 0xFF, 0xFE };
		if (fwrite(UTF8_BOM, sizeof(unsigned char), 2, WriteToFile) != 2)
		{
			fclose(ReadFromFile);
			fclose(WriteToFile);
	...
	{
		unsigned char UTF16_BOM[3] = { 0xEF, 0xBB, 0xBF };
		if (fwrite(UTF16_BOM, sizeof(unsigned char), 3, WriteToFile) != 3)
		{
			fclose(ReadFromFile);
			fclose(WriteToFile);
			remove(WriteToFilePath.c_str());
