...
short WINAPI DLLExport DestroyRunObject(LPRDATA rdPtr, long fast) {
	...
	if(rdPtr->screenDot != NULL){
		delete [] rdPtr->screenDot;
		rdPtr->screenDot = NULL;
	}
	if(rdPtr->screenRender != NULL){
		delete [] rdPtr->screenRender;
		rdPtr->screenRender = NULL;
	}
	delete rdPtr->rRd;
	return 0;
}
...
// HandleRunObject
short WINAPI DLLExport HandleRunObject(LPRDATA rdPtr) {  return rdPtr->rc.rcChanged?REFLAG_DISPLAY:0; }

void renderVoxel(LPRDATA rdPtr);

// DisplayRunObject
short WINAPI DLLExport DisplayRunObject(LPRDATA rdPtr) {
	if(rdPtr->voxelParts != NULL)
		renderVoxel(rdPtr);
