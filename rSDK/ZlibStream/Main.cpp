...
struct Params
{
    TCHAR para_infilename [MAX_PATH];  //Input file name
    TCHAR para_outfilename [MAX_PATH]; //Output file name
...
//Calculate file size
unsigned long file_size(TCHAR *filename) {
   FILE *pFile = NULL;
   if (_tfopen_s(&pFile, filename, _T("rb")))  // Open file
...
bool FileSizeCheck(TCHAR *infilename, LPRDATA rdPtr)
{
    if (file_size(infilename) < 1)
    {
        ThreadSafe_Start();
        rdPtr -> returnstring = _T("Input file size 0 or nonexistent.");
        ThreadSafe_End();
        return true;
    }
    else
...
bool HandleCheck(FILE *file, gzFile gzfile, LPRDATA rdPtr)
{
    ...
    {
        ThreadSafe_Start();
        rdPtr -> returnstring = _T("Input file malfunctioned.");
        gzclose(gzfile);
        ThreadSafe_End();
        return true;
    }
    ...
    {
        ThreadSafe_Start();
        rdPtr -> returnstring = _T("Output file malfunctioned.");
        fclose(file);
        ThreadSafe_End();
        return true;
    }
...
{    
	...
	TCHAR *infilename = new TCHAR [MAX_PATH];             //Get input file name
	_tcscpy_s(infilename, MAX_PATH, pDataArray -> para_infilename);
    TCHAR *outfilename = new TCHAR [MAX_PATH];             //Get output file name
	_tcscpy_s(outfilename, MAX_PATH, pDataArray -> para_outfilename);
	...
	const char * WriteType = "wb9";        //User is using write (overwriting)
    ...
    FILE *infile = _tfopen(infilename, _T("rb"));      //r = read, b = binary
    //gzFile outfile = gzopen(outfilename, WriteType); //a = append (or w=write), b = binary, 9 = max compression

	gzFile outfile = t_gzopen(outfilename, WriteType);
...
{    
	...
	TCHAR * WriteType = _T("wb");        //User is using write (overwriting)
	if (UseAppend) WriteType = _T("ab"); //User is using append

    if (FileSizeCheck(infilename, rdPtr)) {Terminate();}
    //Open handles to both files
    //gzFile infile = gzopen(infilename, "rb"); //r = read, b = binary

	gzFile infile = t_gzopen(infilename, "rb");

    FILE *outfile = _tfopen(outfilename, WriteType); //a = append or w = write, b = binary
    ...
    //Then declare final variable, the buffer
    TCHAR *inbuffer = new TCHAR[tempinbuffersize];
...
ACTION(
    ...
    /* Flags */     0,
    /* Params */    (1, PARAM_NUMBER, _T("Buffer size (1 to 16384 bytes):"))
) {
    ...
    {
        rdPtr -> returnstring = _T("Buffer is an invalid size. Must be between 0 and 16385 (exclusive).");
        rdPtr -> rRd -> CallEvent(1);
    }
    ThreadSafe_End();
}
...
ACTION(
    ...
    /* Flags */     0,
    /* Params */    (3, PARAM_STRING, _T("Input file:"), PARAM_STRING, _T("Output file:"), PARAM_NUMBER, _T("Append if output file already exists? (0 for overwriting, 1 for adding to end)"))
) {
    //Retrieve variables from parameters
    TCHAR *p1 = (TCHAR *) Param(TYPE_STRING);
	...
	_tcscpy_s(Parameters -> para_infilename, MAX_PATH, p1);  //Pass the input file name
    _tcscpy_s(Parameters -> para_outfilename, MAX_PATH, p2); //Pass the output file name
...
EXPRESSION(
    ...
    ThreadSafe_Start();
    tstring temp = rdPtr -> returnstring;
    ThreadSafe_End();
...
EXPRESSION(
	...
	TCHAR sprintfdest[20];
	_stprintf(sprintfdest,_T("%.2f"), temp);
	float temp2 = (float)_tstof(sprintfdest);
	//Voila
    ReturnFloat(temp2);
}
