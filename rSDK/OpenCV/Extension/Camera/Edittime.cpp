...
PROPS_DATA_START()
	PropData_EditNumber(PROPID_XANGLE,PSTR("Angle (x)"),PSTR("The objects x angle.")),
	PropData_EditNumber(PROPID_YANGLE,PSTR("Angle (y)"),PSTR("The objects y angle.")),
	PropData_EditNumber(PROPID_ZANGLE,PSTR("Angle (z)"),PSTR("The objects z angle.")),
...
LPVOID WINAPI DLLExport GetPropValue(LPMV mV, LPEDATA edPtr, UINT nPropID)
{
	...
	switch (nPropID) {
		case PROPID_XANGLE:
			return new CPropIntValue(edPtr->xAngle);
			break;
		case PROPID_YANGLE:
			return new CPropIntValue(edPtr->yAngle);
			break;
		case PROPID_ZANGLE:
			return new CPropIntValue(edPtr->zAngle);
			break;
//
//	// Returns a color.
//	case PROPID_COLOR:
...
void WINAPI DLLExport SetPropValue(LPMV mV, LPEDATA edPtr, UINT nPropID, LPVOID lParam)
{
	...
	switch (nPropID) {
		case PROPID_XANGLE:
			edPtr->xAngle= ((CPropIntValue*)pValue)->m_nValue;
			break;
		case PROPID_YANGLE:
			edPtr->yAngle= ((CPropIntValue*)pValue)->m_nValue;
			break;
		case PROPID_ZANGLE:
			edPtr->zAngle= ((CPropIntValue*)pValue)->m_nValue;
			break;
...
void WINAPI DLLExport EditorDisplay(mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr, RECT FAR *rc)
{
	...
	if ( ps != NULL )		// Do the following if this surface exists
	{
		...
		is.Create(w, h, ps);	// Create a surface implementation from a prototype (frame editor win)
		is.Rectangle(0,0,w,h,RGB(192,192,192),1,RGB(10,10,10));	// and remap palette if necessary

		cSurface myIcon;
		myIcon.Create(32, 32, ps);	// Create a surface implementation from a prototype (frame editor win)

		myIcon.LoadImage(hInstLib, EXO_IMAGE, LI_REMAP);
		myIcon.Blit(is, w/2 - 16, h/2 - 16, BMODE_TRANSP, BOP_COPY, 0);	// Blit the image to the frame editor surface!
