...
void ChangeLastError(std::string NewError,LPRDATA rdPtr)
{
	if (EventLastError.empty())
...
CONDITION(
	...
	if (n >= 0 && n < MAX_SLOTS)
	{
		...
		return !rdPtr->slots[n].bPause &&
			!rdPtr->slots[n].bCompleted;
	}
	else
		return 0;
}
...
EXPRESSION(
	...
	if (n >= 0 && n < MAX_SLOTS)
	{
		...
		{
			char temp[255];
			sprintf_s(temp, "%s", rdPtr->slots[n].pURL.c_str());
			ReturnString(*temp);
		}
		else // If in range and there is a URL
		{
...
EXPRESSION(
	...
	if (n >= 0 && n < MAX_SLOTS) {
		if (rdPtr->slots[n].TotalRead > 0)
			return (size_t)rdPtr->slots[n].TotalRead; // If in range and size saved
		else
		{
...
EXPRESSION(
	...
	if (n >= 0 && n < MAX_SLOTS)
	{
		if (rdPtr->slots[n].TotalRead > 0)
		{
			ReturnFloat(((rdPtr->slots[n].FileSize * 1.0) / (rdPtr->slots[n].TotalRead * 1.0)) * 100.0);
		}
		// If in range and size saved
		else
		{
			ChangeLastError("Download failed or not initiated", rdPtr); // If in range but no size
			ReturnString("");
...
EXPRESSION(
	...
	if (n >= 0 && n < MAX_SLOTS)
	{
		if (rdPtr->slots[n].TotalRead > 0)
		{
			ReturnFloat(((rdPtr->slots[n].TotalRead - rdPtr->slots[n].FileSize) * 1.0) / (rdPtr->slots[n].Speed * 1.0));
		}
		else // If in range and size saved
		{
			ChangeLastError("Download failed, not initiated, or paused", rdPtr); // If in range but no size
			ReturnString("");
		}
...
EXPRESSION(
	...
	char temp[255];
	sprintf_s(temp, "%s.", EventLastError.c_str());
	EventLastError.clear(); // Resets error string
	ReturnStringSafe(temp);
}
